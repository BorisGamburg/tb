...
class CloseLimitMng:
    ...
    def _place_close_order(
        ...
        # Получаем цену для активного/пассивного исполнения лимитного ордера 
        if exit_type == ExitType.ACTIVE:
            price = self.market_service.get_active_price(
                symbol=symbol,
                side=side,
            )
        elif exit_type == ExitType.PASSIVE:
